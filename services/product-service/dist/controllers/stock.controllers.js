...
export class StockController {
    ...
    async reserveStock(req, res, next) {
        ...
        try {
            ...
            await stockService.reserveStock(new Types.ObjectId(versionId), quantity, session);
    ...
    async confirmStock(req, res, next) {
        ...
        try {
            ...
            session.startTransaction();
            await stockService.confirmStock(new Types.ObjectId(versionId), quantity, session);
            await session.commitTransaction();
    ...
    async releaseStock(req, res, next) {
        ...
        try {
            ...
            session.startTransaction();
            await stockService.releaseStock(new Types.ObjectId(versionId), quantity, session);
            await session.commitTransaction();
    ...
    async reStock(req, res, next) {
        try {
            ...
            const stock = await stockService.restock(new Types.ObjectId(versionId), quantity);
            res.status(200).json(stock);
        }
        catch (error) {
            next(error);
    ...
    async correctStock(req, res, next) {
        try {
            ...
            const stock = await stockService.restock(new Types.ObjectId(versionId), total);
            res.status(200).json(stock);
        }
        catch (error) {
            next(error);
