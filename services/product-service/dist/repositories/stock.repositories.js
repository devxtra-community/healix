...
export class StockRepository {
    ...
    async createInitialStock(productVersionId, total, session) {
        return ProductStockModel.findOneAndUpdate({ product_version_id: productVersionId }, {
            $setOnInsert: {
    ...
    async updateStock(versionId, newVersionId, session) {
        ...
        }, { $set: { product_version_id: newVersionId } }, { new: true, session });
    }
    // READ STOCK
    ...
    async releaseStock(productVersionId, quantity, session) {
        ...
        if (!existing) {
            throw new Error(`Stock record not found for versionId: ${productVersionId}`);
        }
        if (existing.reserved < quantity) {
            throw new Error(`Cannot release ${quantity} units — only ${existing.reserved} are reserved for versionId: ${productVersionId}`);
        }
        const stock = await ProductStockModel.findOneAndUpdate({
            product_version_id: productVersionId,
        ...
        if (!stock)
            throw new Error('Stock release failed due to a concurrent update. Please retry.');
        return stock;
    }
    ...
    async confirmStock(productVersionId, quantity, session) {
        ...
        if (!stock)
            throw new Error('Invalid stock confirmation');
        return stock;
    }
    // ADMIN OPERATIONS
