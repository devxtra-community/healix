                ...
                {product.images.length > 1 && (
                  <div className="grid grid-cols-3 gap-3">
                    {product.images.map((img, idx) => (
                      <div
                        key={idx}
                        className="aspect-square rounded-lg overflow-hidden bg-gray-100 border border-transparent hover:border-green-600 cursor-pointer"
                      >
                        <img
              ...
              <div className="bg-white p-5 rounded-xl shadow-sm border border-gray-100 flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="flex items-center gap-1 text-sm font-medium">
                    ...
                    {uiPlaceholders.rating}{' '}
                    <span className="text-gray-400">
                      ({(uiPlaceholders.reviewsCount / 1000).toFixed(1)}k
                      reviews)
                    </span>
                  </div>
              ...
              <div className="flex items-center gap-4 text-sm mb-6">
                <div className="flex items-center text-yellow-400">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i < Math.floor(uiPlaceholders.rating) ? 'fill-current' : 'text-gray-300'}`}
                    />
                  ))}
                </div>
                <span className="font-semibold">{uiPlaceholders.rating}</span>
                <span className="text-gray-400">
                  {uiPlaceholders.reviewsCount} reviews
                </span>
                <span className="text-gray-400">•</span>
                <span className="font-medium">
                  {uiPlaceholders.soldCount} Sold
                </span>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="flex gap-2">
                  {uiPlaceholders.colors.map((color, idx) => (
                    <button
                      key={idx}
                      className="px-4 py-2 border rounded-md text-sm hover:border-green-600 hover:text-green-600 transition-colors"
                    >
                      {color}
                    </button>
                  ))}
              ...
              <div className="mb-8">
                ...
                <div className="flex gap-2">
                  {uiPlaceholders.sizes.map((size, idx) => (
                    <button
                      key={idx}
                      className={`w-10 h-10 flex items-center justify-center border rounded-md text-sm font-medium hover:border-green-600 hover:text-green-600 transition-colors ${idx === 0 ? 'border-gray-300' : ''}`}
                    >
                      {size}
                    </button>
                  ))}
