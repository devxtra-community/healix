...
export default function StripePaymentPage() {
  ...
  useEffect(() => {
    ...
    if (existingScript) {
      ...
      scriptEl.addEventListener('load', onScriptLoad, { once: true });
    ...
    return () => {
      cancelled = true;
      if (scriptEl && onScriptLoad) {
        scriptEl.removeEventListener('load', onScriptLoad);
      }
      if (scriptEl && onScriptError) {
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    setSubmitting(true);
    const returnUrl = `${window.location.origin}/orders${orderId ? `?orderId=${orderId}` : ''}`;
