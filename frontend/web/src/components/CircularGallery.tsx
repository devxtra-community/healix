...
// --- Title Class ---
class Title {
  gl: any; plane: Mesh; renderer: Renderer; text: string; textColor: string; font: string; mesh!: Mesh;
...
class Media {
  ...
  constructor({ geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius = 0, font }: any) {
    Object.assign(this, { geometry, gl, image, index, length, renderer, scene, screen, text, viewport, bend, textColor, borderRadius, font, extra: 0, speed: 0 });
    this.createShader();
    this.createMesh();
    this.createTitle();
    this.onResize();
...
class App {
  ...
  createCamera() { this.camera = new Camera(this.gl); this.camera.position.z = 20; }
  ...
  createMedias(config: any) {
    ...
    this.medias = [...items, ...items].map((data, index) => new Media({
      geometry: this.planeGeometry, gl: this.gl, image: data.image, index, length: items.length * 2,
      renderer: this.renderer, scene: this.scene, screen: this.screen, text: data.text, viewport: this.viewport,
      bend: config.bend, textColor: config.textColor, borderRadius: config.borderRadius, font: config.font
    }));
  }
  ...
  addEventListeners() {
    ...
    this.container.addEventListener('wheel', (e:any) => { this.scroll.target += e.deltaY * 0.05 * this.scrollSpeed; });
    // Touch/Mouse drag logic...
    let isDown = false, start = 0;
    const down = (e: any) => { isDown = true; start = e.touches ? e.touches[0].clientX : e.clientX; this.scroll.pos = this.scroll.current; };
    const move = (e: any) => { if(!isDown) return; const x = e.touches ? e.touches[0].clientX : e.clientX; this.scroll.target = this.scroll.pos + (start - x) * 0.05; };
    const up = () => isDown = false;
    this.container.addEventListener('mousedown', down); this.container.addEventListener('mousemove', move); window.addEventListener('mouseup', up);
    this.container.addEventListener('touchstart', down); this.container.addEventListener('touchmove', move); window.addEventListener('touchend', up);
  }

  destroy() { window.cancelAnimationFrame(this.raf); this.gl.canvas.remove(); }
}
